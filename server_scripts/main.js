...
PlayerEvents.tick(e => {
    ...
    for (const window of data.get("windows")) {
        ...
        if (distance < 1.5 && e.player.getPose() == Pose.CROUCHING && Math.floor(e.player.y) == Math.floor(window.y) + 1 && e.player.getPose() != Pose.SWIMMING) {
    ...
    for (const vent of data.get("vents")) {
        ...
        if (distance < 1.25 && e.player.getPose() == Pose.CROUCHING && Math.floor(e.player.y) == Math.floor(vent.y) && e.player.getPose() != Pose.SWIMMING) {
